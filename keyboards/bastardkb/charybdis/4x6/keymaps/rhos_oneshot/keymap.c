...
// #define CHARYBDIS_AUTO_SNIPING_ON_LAYER U_NAV
...
void eeconfig_init_user(void) {
    ...
    user_config.user_brightness = 17;
...
void process_root_layer_toggle(uint16_t keycode) {
    if (keycode != FN_LCK) return;
    if (user_config.root_layer == U_FBASE) {
        user_config.root_layer = U_BASE;
        save_user_config();
    } else {
        user_config.root_layer = U_FBASE;
        save_user_config();
    }
...
void process_pointing(uint16_t keycode, keyrecord_t *record, bool *continue_processing, bool *layer_updated) {
...
void process_user_brightness_step(uint16_t keycode) {
    if (keycode != U_RGB_B) return;

    uint8_t *brightness = &user_config.user_brightness;
    if (os_LSFT_key.oneshot_state == os_up_unqueued) {
        if (*brightness > (uint8_t)(255u - 17u)) {
            *brightness = 255u;
        } else {
            *brightness = (uint8_t)(*brightness + 17u);
        }
    } else {
        if (*brightness < 17u) {
            *brightness = 0u;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed) {
        process_root_layer_toggle(keycode);
        process_user_brightness_step(keycode);
    }

    bool layer_updated = false;
    process_pointing(keycode, record, &continue_processing, &layer_updated);
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    x += abs(mouse_report.x);
    y += abs(mouse_report.y);
...
static const RGB indicator_colors[COLOR_COUNT] = {
    [COLOR_CAPS] = {255, 0, 0}, // caps active

    [COLOR_NKRO] = {125, 125, 125}, // nkro toggle

    [COLOR_DRG] = {255, 125, 0}, // drg state
    [COLOR_PNT] = {0, 125, 255}, // pnt state

    [COLOR_FN] = {255, 80, 80}, // function (top row lock)
    [COLOR_AS] = {80, 255, 80}, // auto shift(for f1-f12)
    [COLOR_AA] = {80, 80, 255}, // auto algr

    [COLOR_DPI]  = {80, 160, 255}, // visualize dpi
    [COLOR_SDPI] = {160, 80, 255}, // visualize sniping dpi
};

void set_color_user_bright(uint8_t led_index, RGB color) {
    uint8_t v = user_config.user_brightness;

    uint8_t r = (uint16_t)color.r * v / 255;
    uint8_t g = (uint16_t)color.g * v / 255;
    uint8_t b = (uint16_t)color.b * v / 255;

    rgb_matrix_set_color(led_index, r, g, b);
}

void set_slot_color(uint8_t led_index, uint8_t color_idx) {
    if (color_idx >= COLOR_COUNT) return;
    set_color_user_bright(led_index, indicator_colors[color_idx]);
}

void rgb_matrix_show_dpi(void) {
    const uint8_t dpi_leds[16] = {52, 51, 50, 49, 48, 47, 46, 45, 41, 42, 43, 44, 40, 39, 38, 37};
    const uint8_t sdpi_leds[4] = {33, 34, 35, 36};
    uint16_t      cur_dpi      = CHARYBDIS_MINIMUM_DEFAULT_DPI;
    for (uint8_t i = 0; i < 16; ++i) {
        set_slot_color(dpi_leds[i], COLOR_DPI);
        if (cur_dpi >= charybdis_get_pointer_default_dpi()) break;
        cur_dpi += CHARYBDIS_DEFAULT_DPI_CONFIG_STEP;
    }
    uint16_t cur_sdpi = CHARYBDIS_MINIMUM_SNIPING_DPI;
    for (uint8_t i = 0; i < 4; ++i) {
        set_slot_color(sdpi_leds[i], COLOR_SDPI);
        if (cur_sdpi >= charybdis_get_pointer_sniping_dpi()) break;
        cur_sdpi += CHARYBDIS_SNIPING_DPI_CONFIG_STEP;
    }
}
...
bool rgb_matrix_indicators_user(void) {
    ...
    if (host_keyboard_led_state().caps_lock) set_slot_color(51, COLOR_CAPS);

    if (keymap_config.nkro) set_slot_color(52, COLOR_NKRO);

    if (user_config.root_layer != U_BASE) set_slot_color(50, COLOR_FN);

    if (user_config.a_shft_active) set_slot_color(46, COLOR_AS);
